...
class CreateBuyers extends Controller
{
    ...
    {
        // 1. Precise execution checking against all database columns (With Icon Fixes Integrated)
        $validated = $request->validate([
            'user_id'                     => 'nullable|exists:users,id',
            'buyer_ref_number'            => 'required|string|max:100|unique:buyer_profiles,buyer_ref_number',
            'company_icon_path'           => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048', // Patched missing parameter
        ...
        // Store avatar file assets into dedicated buyer directory folder map path partitions
        $iconPath = $request->hasFile('company_icon_path')
            ? $request->file('company_icon_path')->store('buyer_icons', 'public')
            : null;

        // Fallback credential password hashing checks
        $passwordString = $request->filled('password')
            ? Hash::make($request->input('password'))
            : Hash::make('glutobuyer');

        // 3. Save and persist everything to your buyer_profiles table database structure
        BuyerProfile::create([
