<div class="flex-1 mx-auto my-2 p-gutter w-full max-w-[1440px]" x-data="{ currentModalOpen: @entangle('isUploadModalOpen') }">
    ...
    <div class="flex sm:flex-row flex-col justify-between sm:items-center gap-4 mb-8 pb-4 border-b border-outline-variant/30 select-none">
        <div>
            <h2 class="font-headline-lg font-bold text-headline-lg text-primary text-2xl tracking-tight">Compliance Documents Vault</h2>
            <p class="mt-0.5 font-body-sm text-on-surface-variant text-sm">Review verified procurement credentials, download active trade agreements, or append target quality logs checks.</p>
        </div>
        <div class="flex items-center gap-1.5 bg-surface-container-low shadow-inner px-4 py-2 border rounded-xl h-fit font-mono font-bold text-primary text-xs">
            <span class="text-[16px] text-emerald-600 material-symbols-outlined">verified</span> Status: {{ $buyer->status_label }}
        </div>
    </div>
        ...
        @foreach($this->documentMap as $fieldName => $meta)
            ...
            <div class="flex flex-col justify-between bg-white shadow-sm hover:shadow-md p-6 border rounded-[2rem] border-outline-variant/50 min-h-[280px] transition-shadow animate-fadeIn">
                    ...
                    <div class="space-y-2 pr-1 max-h-48 overflow-y-auto hide-scrollbar">
                        @if(filled($filesCollection) && is_array($filesCollection))
                            @foreach($filesCollection as $nodeIndex => $docAsset)
                                <div class="relative flex justify-between items-center bg-surface-container-low hover:bg-surface-container-low/80 p-2.5 pr-10 border rounded-xl text-xs transition-colors animate-fadeIn">
                                    ...
                                    @else
                                        <span class="bg-surface-container-high px-1.5 py-0.5 rounded font-sans font-medium text-[9px] text-on-surface-variant/80 select-none">
                                            Global Profile
                                        </span>
                                    @endif
                ...
                <div class="mt-4 pt-4 border-background border-t w-full select-none">
                    <button type="button" wire:click="openUploadModal('{{ $fieldName }}')" class="flex justify-center items-center gap-1.5 bg-surface-container-high hover:bg-primary/10 py-2.5 rounded-xl w-full font-bold text-primary text-xs transition-colors cursor-pointer">
        ...
        <div @click.outside="currentModalOpen = false; $wire.call('closeUploadModal')"
            ...
            <form wire:submit.prevent="appendDocument" class="space-y-4 p-6 text-xs">
                ...
                <div class="space-y-1.5">
                    ...
                    <p class="pl-0.5 text-[10px] text-on-surface-variant/80">Accepts PDF, DOCX, or Image snapshots up to 5MB capacity.</p>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="associated_product_ref" wire:model.defer="associated_product_ref" class="bg-surface-container-low focus:bg-white px-3 py-2 border rounded-xl border-outline-variant outline-none focus:ring-1 focus:ring-primary w-full font-mono font-bold text-primary" placeholder="e.g., SKU-4902 (Leave blank if profile global)">
                ...
                <div class="flex justify-end items-center gap-2 pt-2 border-background border-t select-none">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="flex justify-center items-center gap-1 bg-primary hover:bg-primary/95 shadow-md px-5 py-2 rounded-xl font-bold text-white cursor-pointer">
                        ...
                        <span wire:loading class="flex items-center gap-1 animate-pulse"><span class="inline-block border-2 border-white border-t-transparent rounded-full w-3 h-3 animate-spin"></span> Syncing array...</span>
                    </button>
                </div>
            </form>
        </div>
