<div class="flex-1 mx-auto my-2 p-gutter w-full max-w-[1440px]"
    x-data="{ currentModalOpen: @entangle('isUploadModalOpen') }">
    ...
    <div
        ...
        <div>
            <h2 class="font-headline-lg font-bold text-headline-lg text-primary text-2xl tracking-tight">Compliance
                Documents Vault</h2>
            <p class="mt-0.5 font-body-sm text-on-surface-variant text-sm">Review verified procurement credentials,
                download active trade agreements, or append target quality logs checks.</p>
        </div>
        <div
            ...
            <span class="text-[16px] text-emerald-600 material-symbols-outlined">verified</span> Status:
            {{ $buyer->status_label }}
        </div>
    </div>
    ...
    <div class="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 max-w-4xl">
        @foreach($this->documentMap as $fieldName => $meta)
            @php $filesCollection = $buyer->{$fieldName} ?? []; @endphp

            <div
                class="flex flex-col justify-between bg-white shadow-sm p-6 border rounded-[2rem] border-outline-variant/50 min-h-[280px] transition-shadow animate-fadeIn">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-center pb-3 border-background border-b select-none">
                        ...
                        <span
                            class="bg-surface-container px-2 py-0.5 border rounded-md font-mono font-bold text-[10px] text-on-surface-variant">
                    ...
                    <div class="space-y-2 pr-1 max-h-48 overflow-y-auto hide-scrollbar">
                        @if(filled($filesCollection) && is_array($filesCollection))
                            @foreach($filesCollection as $nodeIndex => $docAsset)
                                <div
                                    class="relative flex justify-between items-center bg-surface-container-low hover:bg-surface-container-low/80 p-2.5 pr-10 border rounded-xl text-xs transition-colors animate-fadeIn">
                                    <a href="{{ asset('storage/' . $docAsset['file_path']) }}" target="_blank"
                                        class="flex items-center gap-1.5 max-w-[200px] font-bold text-primary hover:underline truncate">
                                        <span class="text-[16px] text-emerald-600 material-symbols-outlined">cloud_download</span>
                                        <span>Download Asset</span>
                                    </a>

                                    <span
                                        class="bg-surface-container-high px-1.5 py-0.5 rounded font-sans font-medium text-[9px] text-on-surface-variant/80 select-none">
                                        Global Profile
                                    </span>

                                    <button type="button" wire:click="removeDocument('{{ $fieldName }}', {{ $nodeIndex }})"
                                        wire:confirm="Are you certain you want to purge this record entry? This removes the associated storage file asset data permanently."
                        ...
                        @else
                            <div
                                class="flex items-center gap-2 bg-background/50 p-4 border border-dashed rounded-xl text-on-surface-variant/50 text-xs italic select-none">
                                <span class="text-[16px] material-symbols-outlined">folder_off</span>
                                <span>No compliance attachments mapped yet.</span>
                            </div>
                        @endif
                    </div>
                </div>

                <div class="mt-4 pt-4 border-background border-t w-full select-none">
                    <button type="button" wire:click="openUploadModal('{{ $fieldName }}')"
    ...
    <div x-show="currentModalOpen" x-cloak
        ...
        <div @click.outside="currentModalOpen = false; $wire.call('closeUploadModal')"
            ...
            x-show="currentModalOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="flex justify-between items-center bg-primary p-5 text-white select-none">
                ...
                <button type="button" @click="currentModalOpen = false; $wire.call('closeUploadModal')"
                    class="hover:bg-white/10 p-1.5 rounded-full transition-colors cursor-pointer">
            ...
            <form wire:submit.prevent="appendDocument" class="space-y-4 p-6 text-xs">
                <div
                    class="bg-surface-container-low/50 p-3 border border-dashed rounded-xl font-medium text-on-surface-variant select-none">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="file" wire:model="uploaded_file" required
                        class="block hover:file:bg-primary/20 file:bg-primary/10 file:mr-3 file:px-3 file:py-1.5 file:border-0 file:rounded-xl w-full file:font-semibold text-on-surface-variant file:text-primary file:text-xs">
                    <p class="mt-1 pl-0.5 text-[10px] text-on-surface-variant/80">Accepts PDF, DOCX, or Image snapshots
                        up to 5MB capacity.</p>
                    @error('uploaded_file') <span
                        class="block pl-0.5 font-bold text-[10px] text-red-500 animate-fadeIn">{{ $message }}</span>
                    @enderror
                </div>

                <div class="flex justify-end items-center gap-2 pt-4 border-background border-t select-none">
                    <button type="button" @click="currentModalOpen = false; $wire.call('closeUploadModal')"
                        class="bg-surface-container hover:bg-surface-container-high px-4 py-2 rounded-xl font-bold text-on-surface-variant cursor-pointer">Dismiss</button>
                    <button type="submit" wire:loading.attr="disabled"
                        class="flex justify-center items-center gap-1 bg-primary hover:bg-primary/95 shadow-md px-5 py-2 rounded-xl font-bold text-white cursor-pointer">
