<div class="flex-1 mx-auto my-6 p-gutter w-full">
    ...
    <div class="flex justify-between items-center mb-8 pb-6 border-b border-outline-variant/30">
        ...
        <a href="{{ route('admin.suppliers.manage') }}" wire:navigate class="flex items-center gap-1.5 bg-surface-container-high hover:bg-surface-container-highest px-4 py-2.5 rounded-xl font-label-md font-bold text-on-surface-variant text-xs transition-all cursor-pointer">
    ...
    <form action="{{ route('admin.buyers.store') }}" method="post" enctype="multipart/form-data"
        ...
        <div x-show="activeStep === 1" class="space-y-6 bg-white shadow-sm p-8 border rounded-[2rem] border-outline-variant/40 animate-fadeIn">
            ...
            <div class="gap-6 grid grid-cols-1 md:grid-cols-3">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="company_registration_number" value="{{ old('company_registration_number') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm">
                </div>

                <div class="space-y-1.5">
                    <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">VAT / Tax ID Number</label>
                    <input type="text" name="vat_tax_id_number" value="{{ old('vat_tax_id_number') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm" placeholder="TIN Number">
                </div>

                <div class="space-y-1.5">
                    <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Year Established</label>
                    <input type="number" name="year_established" value="{{ old('year_established') }}" min="1800" max="{{ date('Y') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm" placeholder="YYYY">
                </div>

                <div class="space-y-1.5 md:col-span-2">
                    <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Nature of Business Operation</label>
                    <input type="text" name="nature_of_business" value="{{ old('nature_of_business') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm" placeholder="e.g., Wholesale Distributor, Retail Chain, Procurement Agency">
                </div>

                <div class="space-y-1.5">
                    <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Corporate Website URL</label>
                    <input type="text" name="company_website" value="{{ old('company_website') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm" placeholder="www.company.com">
                </div>
                ...
                <div class="space-y-1.5 md:col-span-3 pt-4 border-t border-dashed border-outline-variant/40">
                    <div class="space-y-1.5 max-w-md">
                        ...
                        <input type="text" name="password" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-mono text-sm" placeholder="Leave blank to use baseline 'glutobuyer' fallback value">
        ...
        <div x-show="activeStep === 2" x-cloak class="space-y-6 bg-white shadow-sm p-8 border rounded-[2rem] border-outline-variant/40 animate-fadeIn">
            ...
            <div class="gap-6 grid grid-cols-1 md:grid-cols-2">
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="rep_nationality" value="{{ old('rep_nationality') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm">
                </div>

                <div class="space-y-1.5">
                    <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Identity Document (ID / Passport) Number</label>
                    <input type="text" name="rep_id_passport_number" value="{{ old('rep_id_passport_number') }}" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm">
                </div>

                <div class="space-y-1.5 md:col-span-2">
                    <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Official Office Operations Physical Address</label>
                    <textarea name="office_address" rows="3" class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant outline-none w-full font-medium text-sm">{{ old('office_address') }}</textarea>
                </div>
            </div>
        ...
        <div x-show="activeStep === 3" x-cloak class="space-y-6 bg-white shadow-sm p-8 border rounded-[2rem] border-outline-variant/40 animate-fadeIn">
            ...
            <p class="-mt-3 text-on-surface-variant text-xs">Attach active corporate records. Link them to a specific system product identifier if required.</p>
            ...
            <div class="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 text-xs">
                ...
                @foreach($buyerVaultFields as $field => $label)
                    <div class="flex flex-col justify-between space-y-3 bg-surface-container-lowest shadow-sm p-5 border rounded-2xl border-outline-variant/60">
                        <div>
                        ...
                        <div class="space-y-1 pt-1.5 border-background border-t">
                            <label class="block font-bold text-[10px] text-on-surface-variant uppercase tracking-wider">Associated Product Ref (Optional)</label>
                            <input type="text" name="{{ $field }}_prod_ref" value="{{ old($field . '_prod_ref') }}" class="bg-white px-3 py-1.5 border rounded-lg border-outline-variant outline-none focus:ring-1 focus:ring-primary w-full font-mono text-primary text-xs" placeholder="e.g., PROD-7104">
                        </div>
                    </div>
                @endforeach
            </div>
