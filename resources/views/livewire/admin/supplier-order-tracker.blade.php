<div class="flex-1 mx-auto my-2 p-gutter w-full max-w-[1440px]">
    ...
    <div class="flex sm:flex-row flex-col justify-between sm:items-center gap-4 mb-6 pb-4 border-b border-outline-variant/30 select-none">
        <div>
            ...
            <h2 class="font-headline-lg font-bold text-headline-lg text-primary text-2xl tracking-tight">Vendor Negotiation Log Board</h2>
            <p class="mt-0.5 font-body-sm text-on-surface-variant text-sm">Coordinate contract specifications, manage target pricing criteria adjustments, and trace active communication layers.</p>
        </div>
        ...
        <div class="space-y-6 lg:col-span-1 text-xs">
            <div class="space-y-4 bg-white shadow-sm p-6 border rounded-[2rem] border-outline-variant/50">
                ...
                <div class="space-y-3 font-semibold text-on-surface-variant">
                    <div>
                        <span class="block text-outline font-bold text-[10px] uppercase select-none">Fulfilling Supplier Enterprise</span>
                        <span class="block mt-0.5 font-bold text-primary text-sm">{{ $order->supplier->company_name ?? 'Wholesale Vendor Profile' }}</span>
                    </div>
                    <div>
                        <span class="block text-outline font-bold text-[10px] uppercase select-none">Consolidated Contract Price</span>
                        <strong class="block font-mono text-emerald-800 text-base select-all">{{ $order->currency }} {{ number_format($order->grand_total_amount, 2) }}</strong>
                    </div>
                    <div>
                        <span class="block text-outline font-bold text-[10px] uppercase select-none">Global Milestones Flags</span>
                        <div class="flex items-center gap-1 mt-1 font-mono font-bold text-[10px] uppercase">
                            <span class="bg-surface-container shadow-inner px-2 py-0.5 border rounded-md text-primary">{{ $order->order_status }}</span>
                            <span class="bg-surface-container shadow-inner px-2 py-0.5 border rounded-md text-on-surface-variant/80">{{ $order->shipment_status }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white shadow-sm p-6 border rounded-[2rem] border-outline-variant/50 select-none">
                <span class="block mb-3 text-outline font-bold text-[10px] uppercase">Itemized Contract Lines ({{ count($order->order_items ?? []) }})</span>
                        ...
                        @foreach($order->order_items as $subItem)
                            <div class="flex justify-between items-center bg-background p-2.5 border rounded-xl border-outline-variant/30 text-[11px]">
                                <div class="max-w-[150px] truncate">
                                    ...
                                    <span class="block mt-0.5 font-mono text-[10px] text-on-surface-variant">Rate: {{ $order->currency }}{{ number_format($subItem['negotiated_price_per_unit'], 2) }}</span>
                                </div>
                                <span class="bg-white shadow-sm px-2 py-0.5 border rounded-md font-mono font-bold text-[10px] text-primary">{{ number_format($subItem['order_quantity']) }} pcs</span>
                            </div>
                        @endforeach
                    @endif
                </div>
        ...
        <div class="space-y-4 lg:col-span-2">
            ...
            <div class="flex flex-col space-y-4 bg-dots-pattern bg-white shadow-sm p-6 border rounded-[2rem] border-outline-variant/40 h-[460px] overflow-y-auto hide-scrollbar">
                @forelse($messages as $msg)
                    @php
                        // Identify message origin using your clean corrected foreignId logic mappings
                    ...
                    <div class="flex flex-col {{ $isAdminMsg ? 'items-end' : 'items-start' }} w-full animate-fadeIn">
                        ...
                        <div class="max-w-[85%] rounded-[1.5rem] px-5 py-3 text-xs leading-relaxed font-medium shadow-sm border
                            ...
                            @if(!empty($msg->flagged_fields_or_docs))
                                <div class="mt-3 pt-2 border-t {{ $isAdminMsg ? 'border-white/10' : 'border-primary/10' }} select-none">
                                    <span class="block font-bold text-[9px] uppercase tracking-wider mb-1 flex items-center gap-0.5 {{ $isAdminMsg ? 'text-red-100' : 'text-red-600' }}">
                                        <span class="text-[13px] material-symbols-outlined">warning</span> Targeted Invalidation Action Flags Appended:
                                    </span>
                ...
                @empty
                    <div class="flex flex-col flex-1 justify-center items-center text-on-surface-variant/60 text-center italic select-none">
                        ...
                        <p class="mx-auto mt-0.5 max-w-xs text-[11px]">No operational updates posted to this tracking ledger node workspace yet.</p>
                    </div>
                @endforelse
            </div>
            ...
            <div class="space-y-4 bg-white shadow-sm p-6 border rounded-[2rem] border-outline-variant/40">
                <div class="gap-6 grid grid-cols-1 sm:grid-cols-3 font-medium text-on-surface-variant text-xs">
                    ...
                    <div class="space-y-1 sm:col-span-2">
                        ...
                        <div class="relative">
                            <textarea wire:model.defer="replyMessage" rows="6" required class="bg-surface-container-low focus:bg-white py-3 pr-16 pl-4 border rounded-xl outline-none focus:ring-1 focus:ring-primary w-full font-sans font-medium text-xs leading-relaxed" placeholder="Type your contract feedback logs notes directly to the supplier panel..."></textarea>
