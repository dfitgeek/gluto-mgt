<div class="flex-1 mx-auto my-2 p-gutter w-full max-w-[1440px]" x-data="{ invoiceOverlayOpen: @entangle('isInvoiceModalOpen') }">
    ...
    <div class="flex sm:flex-row flex-col justify-between sm:items-center gap-4 mb-6 pb-4 border-b border-outline-variant/30 select-none">
        <div>
            <h2 class="font-headline-lg font-bold text-headline-lg text-primary text-2xl tracking-tight">Master Buyer Sourcing Quotes Matrix</h2>
            <p class="mt-0.5 font-body-sm text-on-surface-variant text-sm">Oversee all multi-item B2B RFQ packages platform-wide, audit buyer payment wire slips, dispatch billing proformas, and coordinate shipment tracks.</p>
        </div>
    </div>
    ...
    <div class="mb-6 max-w-md select-none">
        <div class="relative w-full">
            ...
            <input wire:model.live.debounce.250ms="search" type="text"
                ...
                placeholder="Search quotes by Voucher ID ticket or specific item name...">
        </div>
    </div>
    ...
    <div class="space-y-6">
        @forelse($quotes as $quote)
            <div wire:key="admin-general-quote-card-{{ $quote->id }}" class="flex flex-col justify-between bg-white shadow-sm hover:shadow-md p-6 border rounded-[2rem] border-outline-variant/50 overflow-hidden transition-shadow animate-fadeIn">
                ...
                <div class="flex flex-wrap justify-between items-center gap-4 pb-3 border-background border-b text-xs select-none">
                    <div class="flex items-center gap-3">
                        <span class="bg-primary/5 shadow-inner px-3 py-1 border border-primary/10 rounded-lg font-mono font-bold text-primary text-sm">
                            Voucher ID: {{ $quote->order_ref_number }}
                        </span>
                        ...
                        <span class="bg-secondary/5 px-2 py-0.5 border rounded font-bold text-[10px] text-primary">
                            Buyer Profile ID Reference: #{{ $quote->buyer_profile_id }}
                        </span>
                    </div>

                    <div class="flex flex-wrap items-center gap-4">
                        <div class="flex items-center gap-1.5 font-bold text-[11px] text-on-surface-variant">
                            <span>Quote Milestone:</span>
                ...
                <div class="gap-6 grid grid-cols-1 lg:grid-cols-3 my-4 font-semibold text-on-surface-variant text-xs">
                    <div class="space-y-2 bg-surface-container-low/40 p-4 border rounded-2xl border-outline-variant/30 select-none">
                        <span class="block text-outline font-bold text-[10px] uppercase tracking-wider">Buyer Details & Route Parameters</span>
                        <div class="space-y-1 font-medium text-[11px] text-primary">
                            <div class="truncate">Company: <strong class="text-xs">{{ $quote->buyer->company_name ?? 'Corporate Account' }}</strong></div>
                            <div>Destination: <strong>{{ $quote->destination_country }} ({{ $quote->incoterms_preferred ?? 'FOB' }})</strong></div>
                            <div>Shipping Vector: <strong>{{ $quote->preferred_shipping_method }}</strong></div>
                        </div>
                    </div>

                    <div class="space-y-2 bg-surface-container-low/40 p-4 border rounded-2xl border-outline-variant/30 select-none">
                        <span class="block text-outline font-bold text-[10px] uppercase tracking-wider">Commercial Transaction Parameters</span>
                        <div class="space-y-1 font-medium text-[11px] text-primary">
                            <div>Preferred Escrow Term: <strong>{{ $quote->preferred_payment_method ?? 'Bank Wire' }}</strong></div>
                            <div>Est. Monthly Volume: <strong>{{ $quote->estimated_monthly_volume ?? 'Not Listed' }}</strong></div>
                    ...
                    <div class="flex flex-col justify-center bg-surface-container-low/80 p-4 border border-dashed rounded-2xl border-outline-variant/80 min-h-[90px] text-left select-none">
                        <span class="block text-outline font-bold text-[10px] uppercase tracking-wider">Consolidated Package Price Aggregate</span>
                        ...
                        <span class="block mt-1 font-sans text-[10px] text-on-surface-variant/70">Total Lines Registered: <strong>{{ count($quote->quotation_items ?? []) }} elements</strong></span>
                    </div>
                </div>
                ...
                <div class="bg-white my-2 border rounded-2xl overflow-hidden select-none">
                    <div class="flex items-center gap-1 bg-surface-container px-4 py-2 border-b border-outline-variant/30 font-bold text-[11px] text-primary uppercase tracking-wider">
                        <span class="text-[16px] material-symbols-outlined replace_icon_tag">list_alt</span> Itemized Quotation Details Ledger
                    </div>
                    ...
                    <div class="gap-3 grid grid-cols-1 md:grid-cols-2">
                        <div class="flex justify-between items-center gap-2 bg-white shadow-sm p-3 border rounded-xl">
                            <span class="flex items-center gap-1.5 text-on-surface truncate select-none">
                                ...
                                <span>Official Sourcing Commercial Invoice Asset</span>
                            </span>
                        ...
                        <div class="flex flex-col justify-center gap-1.5 bg-white shadow-sm p-3 border rounded-xl">
                            <div class="flex justify-between items-center pb-1 border-b select-none">
                                <span class="flex items-center gap-1.5 font-bold text-on-surface text-primary">
                                    ...
                                    <span>Client Remitted Payment Receipts Audit Trail</span>
                                </span>
                                    ...
                                    @foreach($quote->payment_meta['receipts'] as $receiptNode)
                                        <div class="flex justify-between items-center bg-surface-container-low p-1.5 border rounded-lg text-[11px]">
                                            <a href="{{ asset('storage/' . $receiptNode['file_path']) }}" target="_blank" class="flex items-center gap-1 max-w-[280px] font-bold text-primary hover:underline truncate">
                                                ...
                                                <span class="truncate">{{ $receiptNode['file_name'] ?? 'View Payment Slip' }}</span>
                                            </a>
                            ...
                            @else
                                <div class="p-1 text-[11px] text-on-surface-variant/40 italic select-none">No payment verification receipt vouchers logged yet.</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap justify-end items-center gap-3 mt-2 pt-4 border-background border-t select-none">
                    <button type="button" wire:click="openInvoiceModal({{ $quote->id }})"
                        ...
                        {{ isset($quote->payment_meta['supplier_invoice']) ? 'Replace Commercial Invoice' : 'Attach Commercial Invoice' }}
                    </button>
        ...
        @empty
            <div class="bg-white p-16 border border-dashed rounded-[2.5rem] text-on-surface-variant text-center italic select-none">
                ...
                <p class="font-bold text-primary text-sm not-italic">No Sourcing RFQs Initialized</p>
                <p class="mx-auto mt-0.5 max-w-sm text-xs">There are no global custom quotations package manifest forms matching your current lookup criteria registered.</p>
            </div>
        @endforelse
    </div>
        ...
        <div @click.outside="invoiceOverlayOpen = false; $wire.call('closeInvoiceModal')" class="space-y-4 bg-white p-6 rounded-[2.5rem] w-full max-w-md overflow-hidden animate-fadeIn transform">
            ...
            <div class="flex justify-between items-center bg-primary -m-6 mb-4 p-5 text-white select-none">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="font-bold text-sm tracking-tight">Dispatch Platform Sourcing Invoice</h3>
                </div>
            ...
            <form wire:submit.prevent="uploadAdminInvoice" class="space-y-4 font-medium text-on-surface-variant text-xs">
                <p class="leading-relaxed">Select the itemized enterprise commercial statement invoice document (PDF formatting layout sheet or snapshot copy) for this specific multi-item contract package.</p>
                ...
                <div class="flex justify-end items-center gap-2 pt-3 border-background border-t font-bold select-none">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="flex items-center gap-1 bg-primary hover:bg-primary/95 shadow-md px-5 py-2 rounded-xl text-white cursor-pointer">
                        ...
                        <span wire:loading class="flex items-center gap-1 animate-pulse"><span class="inline-block border-2 border-white border-t-transparent rounded-full w-3 h-3 animate-spin"></span> Syncing files...</span>
                    </button>
                </div>
            </form>
        </div>
