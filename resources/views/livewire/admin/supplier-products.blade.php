<div class="flex-1 mx-auto my-2 p-gutter w-full max-w-[1440px]" x-data="{ specModalOpen: false }">
    ...
    <div class="flex md:flex-row flex-col justify-between md:items-center gap-4 mb-6 pb-6 border-b border-outline-variant/30">
        ...
        <div class="flex flex-wrap items-center gap-2.5">
            @if(count($basket) > 0)
                <button type="button" wire:click="proceedToCheckout" class="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-600/95 shadow px-5 py-2.5 rounded-xl font-label-md font-bold text-white text-xs active:scale-95 transition-all animate-fadeIn cursor-pointer">
                    <span class="text-[18px] material-symbols-outlined">shopping_cart_checkout</span>
                    Checkout Order ({{ count($basket) }} items)
                </button>
            @endif
    ...
    <div class="flex items-center bg-white shadow-sm mb-8 p-4 border rounded-2xl border-outline-variant/60 max-w-xl select-none">
    ...
    <div class="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        @forelse($products as $product)
            @php $inBasket = isset($basket[$product->id]); @endphp
            <div wire:key="admin-prod-card-{{ $product->id }}"
                class="group flex flex-col justify-between bg-white shadow-sm hover:shadow-md p-5 border rounded-[2rem] {{ $inBasket ? 'border-emerald-500 ring-1 ring-emerald-500' : 'border-outline-variant/50' }} overflow-hidden transition-all animate-fadeIn duration-300">
                ...
                <div>
                    ...
                    <div class="space-y-1 mb-4 px-1">
                        ...
                        <div class="flex items-baseline gap-1 pt-1.5 select-none">
                ...
                <div class="flex items-center gap-2 pt-2 border-background border-t w-full select-none">
                    <button type="button" wire:click="inspectProductSpecs({{ $product->id }})" class="flex justify-center items-center bg-surface-container hover:bg-surface-container-high p-2.5 border rounded-xl text-primary transition-colors cursor-pointer" title="Inspect engineering/logistical spec sheets parameters">
                        <span class="text-[18px] material-symbols-outlined">visibility</span>
                    </button>

                    <button type="button" wire:click="toggleAddToBasket({{ $product->id }})"
    ...
    <div x-on:open-admin-product-modal.window="specModalOpen = true; document.body.classList.add('overflow-hidden')"
        ...
        @if($activeProductSpec)
            <div @click.outside="specModalOpen = false; document.body.classList.remove('overflow-hidden'); $wire.call('closeSpecModal')"
                 ...
                 x-data="{ modalActiveTab: 'specs_general' }"
                 x-show="specModalOpen"
