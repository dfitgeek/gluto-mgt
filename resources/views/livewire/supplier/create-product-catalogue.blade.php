<div class="flex-1 mx-auto my-4 p-gutter w-full max-w-6xl" x-data="{ creationMethod: 'manual' }">

    <div
        class="flex sm:flex-row flex-col justify-between sm:items-center gap-4 mb-6 pb-4 border-b border-outline-variant/30">
        <div>
            <h2 class="font-headline-lg font-bold text-headline-lg text-primary text-2xl">Expand Inventory Catalogue
            </h2>
            <p class="mt-1 font-body-sm text-on-surface-variant text-sm">Add item specifications to your corporate index
                catalog manually or upload bulk asset datasheets.</p>
        </div>
        ...
        <div class="flex bg-surface-container p-1 border rounded-xl border-outline-variant/20 w-fit">
            <button type="button" @click="creationMethod = 'manual'"
                ...
                <span class="flex items-center gap-1.5"><span
                        class="text-[16px] material-symbols-outlined">edit_note</span> Manual Input</span>
            </button>
            <button type="button" @click="creationMethod = 'upload'"
                ...
                <span class="flex items-center gap-1.5"><span
                        class="text-[16px] material-symbols-outlined">cloud_upload</span> Upload PDF / XLSX Sheet</span>
            </button>
        </div>
    </div>
    ...
    <form action="{{ route('supplier.products.store') }}" method="post" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="gap-6 grid grid-cols-1 md:grid-cols-3">
                    <div class="space-y-1.5 md:col-span-2">
                        <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Product Item Name
                            *</label>
                        <input type="text" name="product_name" :disabled="creationMethod !== 'manual'"
                    ...
                    <div class="space-y-1.5">
                        <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Product Category
                    ...
                    <div class="space-y-1.5">
                        <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">UPC / EAN Barcode
                            Code</label>
                        <input type="text" name="ean_upc_code" :disabled="creationMethod !== 'manual'"
                    ...
                    <div class="space-y-1.5">
                        <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Product Expected Shelf
                            Life</label>
                        <input type="text" name="shelf_life" :disabled="creationMethod !== 'manual'"
                            class="bg-surface-container-low focus:bg-white px-4 py-2.5 border rounded-xl border-outline-variant outline-none focus:ring-1 focus:ring-primary w-full font-medium text-sm"
                    ...
                    <div class="space-y-1.5 md:col-span-3">
                        ...
                        <textarea name="product_description" rows="3" :disabled="creationMethod !== 'manual'"
                            class="bg-surface-container-low focus:bg-white px-4 py-2.5 border rounded-xl border-outline-variant outline-none focus:ring-1 focus:ring-primary w-full text-sm"
                            placeholder="Detail component metrics, manufacturing language criteria, and geographic extraction origin notes..."></textarea>
                    </div>
                </div>

                <div class="space-y-2 pt-4 border-t border-dashed border-outline-variant/60">
                    <label class="block pl-0.5 font-bold text-primary text-xs">Upload Product Catalog Images (Select
                        multiple up to 4 files) *</label>
                    <input type="file" name="product_images[]" :disabled="creationMethod !== 'manual'"
                        :required="creationMethod === 'manual'"
                        class="block hover:file:bg-primary/20 file:bg-primary/10 file:mr-4 file:px-4 file:py-2 file:border-0 file:rounded-xl w-full file:font-semibold text-on-surface-variant file:text-primary text-xs file:text-xs"
                        accept="image/*" multiple>
                    <p class="pl-0.5 text-[10px] text-on-surface-variant">Accepts PNG, JPG formats up to 2MB per file
                        instance. Images will be packed into standard JSON collection stacks array.</p>
                </div>
            </div>
                ...
                <div class="gap-6 grid grid-cols-1 md:grid-cols-3">
                    <div class="space-y-1.5">
                        <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Pieces Per Case
                            (Pcs/Case)</label>
                        <input type="number" name="pcs_per_case" value="0" :disabled="creationMethod !== 'manual'"
                    ...
                    <div class="space-y-1.5">
                        <label class="block pl-0.5 font-bold text-on-surface-variant text-xs">Pricing Structure
                            Configuration</label>
                        <input type="text" name="pricing_structure_type" :disabled="creationMethod !== 'manual'"
                            class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant w-full text-sm"
                            placeholder="e.g., Volume Bulk Tier Rates">
                    </div>
                    ...
                    <div
                        ...
                        <input type="checkbox" name="ability_to_provide_samples" value="1" id="samples_toggle"
                            :disabled="creationMethod !== 'manual'" class="rounded w-4 h-4 text-primary">
                    ...
                    <div class="space-y-1.5 md:col-span-3">
                        ...
                        <input type="text" name="full_truckload_details" :disabled="creationMethod !== 'manual'"
                            class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant w-full text-sm"
                            placeholder="e.g., Max 24 Pallets weight cap / 44,000 lbs freight container limits">
                    </div>
                    <div class="space-y-1.5 md:col-span-3">
                        ...
                        <textarea name="payment_terms" rows="2" :disabled="creationMethod !== 'manual'"
                            class="bg-surface-container-low px-4 py-2.5 border rounded-xl border-outline-variant w-full text-sm"
                            placeholder="e.g., Net 30 days upon delivery receipt validation or Letter of Credit at sight..."></textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-show="creationMethod === 'upload'" x-cloak class="animate-fadeIn">
            <div class="space-y-6 bg-white shadow-sm p-8 border rounded-[2rem] border-outline-variant/40">
                ...
                <p class="-mt-3 text-on-surface-variant text-xs">If you maintain your product catalog inventory records
                    inside a standardized sheet manual, attach the layout file below. Our administrative compliance
                    operations panel will parse your asset sheets internally.</p>
                ...
                <div
                    ...
                    <div class="space-y-1">
                        <label class="block font-label-md font-bold text-primary text-sm cursor-pointer">
                            <span
                                class="inline-block bg-primary hover:bg-primary/95 shadow-sm px-5 py-2 rounded-xl text-white text-xs transition-all">Choose
                                Catalogue Document</span>
                            <input type="file" name="product_catalogue" :disabled="creationMethod !== 'upload'"
                                :required="creationMethod === 'upload'" class="hidden" accept=".pdf,.xls,.xlsx"
                        ...
                        <p id="catalog-file-info" class="mt-2 font-mono font-bold text-secondary text-xs tracking-wide">
                            No template datasheet chosen</p>
                    </div>
                    <p class="text-[10px] text-on-surface-variant/80">Supports PDF spreadsheets, Excel XLSX workbook
                        profiles frameworks up to 10MB.</p>
                </div>
                ...
                <div class="gap-4 grid grid-cols-1 md:grid-cols-2 pt-4 border-t border-dashed">
                    <div class="space-y-1">
                        ...
                        <input type="text" name="bulk_product_name" :disabled="creationMethod !== 'upload'"
                            ...
                            placeholder="e.g., Q3 Corporate Flour Commodity Catalogue Stack">
                    </div>
                    <div class="space-y-1">
                        <label class="block font-bold text-on-surface-variant text-xs">Primary Category Cluster Mapping
                            *</label>
                        <input type="text" name="bulk_product_category" :disabled="creationMethod !== 'upload'"
                            :required="creationMethod === 'upload'"
                            class="bg-surface-container-low px-4 py-2 border rounded-xl border-outline-variant w-full font-medium text-sm"
                            placeholder="e.g., Milling Grains Stock Mix">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="flex items-center gap-2 font-semibold text-on-surface-variant text-xs">
                ...
                <span>Active Vendor session guard token verified:
                    {{ auth('supplier')->user()->supplier_ref_number }}</span>
            </div>
            <button type="submit"
