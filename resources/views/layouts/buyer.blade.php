...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&family=Geist:wght@400;600;700&family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200&display=swap" rel="stylesheet" />
    ...
    <header class="top-0 z-50 sticky bg-surface dark:bg-surface-dim shadow-[0px_4px_20px_rgba(6,78,59,0.05)] w-full">
        <div class="flex justify-between items-center mx-auto px-container-padding w-full h-20">
            <div class="flex items-center gap-4">
                <button class="lg:hidden hover:bg-surface-container-high p-2 rounded-lg text-on-surface-variant material-symbols-outlined">
                    menu
                </button>
                <img src="{{ asset('images/logo.png') }}" alt="Gluto Logo" class="h-[35px] md:h-[50px]">

                <div class="flex flex-col justify-items-start items-start gap-2">
                    <span class="hidden sm:inline-block bg-primary/10 px-3 py-1 border rounded-lg font-bold text-[11px] text-primary uppercase tracking-wider">
                        Buyer Terminal
                    </span>
                    <div class="hidden md:flex flex-col justify-center items-end">
                        @if (auth('buyer')->user()->status_label === 'Verified Buyer')
                            <span
...
@endif
                    ...
                    @if (auth('buyer')->check())
                        <div class="flex items-center gap-3">
                            <div class="hidden sm:block text-right">
                                <p class="max-w-[180px] font-label-md font-bold text-label-md text-on-surface text-sm truncate">
                                    {{ auth('buyer')->user()->company_name }}
                                </p>
                                <p class="mt-0.5 max-w-[180px] font-body-sm text-[11px] text-on-surface-variant/90 truncate">
                                    Buyer Representative: {{ auth('buyer')->user()->rep_full_name }}
                                </p>
                            </div>


                            <img alt="Corporate Brand Logo"
                                class="bg-white shadow-inner border-2 border-primary-fixed rounded-xl w-10 h-10 object-cover"
                                src="{{ auth('buyer')->user()->company_icon_path ? asset('storage/' . auth('buyer')->user()->company_icon_path) : 'https://www.w3schools.com/howto/img_avatar.png' }}" />
                        </div>
                    @endif
                    ...
                    <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                        ...
                        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150"
                            ...
                            <div class="mb-1 px-4 py-2 border-b border-outline-variant/30">
                                <p class="font-bold text-[10px] text-on-surface-variant/80 uppercase tracking-wider">Account Settings</p>
                            </div>

                            <a href="{{ route('buyer.profile') }}" wire:navigate @click="open = false"
                            ...
                            <div class="mt-1 pt-1 border-t border-outline-variant/20">
                                <form action="{{ route('buyer.logout') }}" method="POST" class="w-full">
                                    @csrf
                                    <button type="submit"
    ...
    <div class="relative flex flex-1 items-stretch w-full">
        ...
        <aside class="hidden top-20 z-40 sticky lg:flex flex-col flex-shrink-0 bg-surface-container-lowest border-r border-outline-variant w-[280px] h-[calc(100vh-80px)] overflow-y-auto">
            <div class="flex items-center gap-2 my-6 px-6">
                <div class="flex justify-center items-center bg-primary-container rounded-lg w-10 h-10 text-white">
                    <span class="text-primary-fixed material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">shopping_cart</span>
                </div>
                <div>
                    <h2 class="font-headline-md font-bold text-headline-md text-primary text-base tracking-tight">Buyer Terminal</h2>
                    <p class="font-bold text-[10px] text-on-surface-variant/70 uppercase tracking-wider">Ref: {{ auth('buyer')->user()->buyer_ref_number }}</p>
                </div>
            </div>

            <nav class="flex-1 space-y-1 px-2">
                <a href="{{ route('buyer.dashboard') }}"
                    class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all {{ request()->routeIs('buyer.dashboard') ? 'bg-secondary-container text-on-secondary-container font-bold' : 'text-on-surface-variant hover:bg-surface-container-high' }}">
                ...
                <a href="{{ route('buyer.order') }}"
                    class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all {{ request()->routeIs('buyer.products*') ? 'bg-secondary-container text-on-secondary-container font-bold' : 'text-on-surface-variant hover:bg-surface-container-high' }}">
               ...
               {{--  <a href="#"
                    class="group flex items-center gap-3 px-4 py-3 rounded-lg transition-all {{ request()->routeIs('buyer.orders.index*') ? 'bg-secondary-container text-on-secondary-container font-bold' : 'text-on-surface-variant hover:bg-surface-container-high' }}">
                    <span class="material-symbols-outlined">receipt_long</span>
            ...
            <div class="space-y-1 mt-auto p-4 border-background border-t">
                <form action="{{ route('buyer.logout') }}" method="POST" class="group flex items-center gap-3 hover:bg-red-50/60 px-4 py-3 rounded-xl w-full text-red-600 transition-all">
                    @csrf
                    <span class="material-symbols-outlined">logout</span>
                    <button type="submit" class="font-label-md font-bold text-label-md text-xs cursor-pointer">
                        Sign Out Workspace
                    </button>
                </form>
            </div>
